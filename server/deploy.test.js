...
test('installed systemd service uses the selected prefix and a runtime outside the user home', () => {
  ...
  const logFile = '/var/log/custom/multi-cpa.log'
  const renderedWithLogFile = renderControllerUnit(template, '/opt/custom', dataDir, logFile)
  assert.match(renderedWithLogFile, /ReadWritePaths=\/home\/vesoft\/cpa\/\.multi-cliproxyapi \/var\/log\/custom\/multi-cpa\.log/)
  assert.match(renderedWithLogFile, /--runtime systemd --log-file "\/var\/log\/custom\/multi-cpa\.log"/)
  assert.match(renderedWithLogFile, /EnvironmentFile=-\/etc\/multi-cliproxyapi\/controller\.env/)
  assert.throws(() => renderControllerUnit(template, '/opt/custom', dataDir, 'relative'), /absolute/)
  ...
  assert.match(binaryInstaller, /LOG_FILE=\$\{MULTI_CPA_LOG_FILE:-\/var\/log\/multi-cpa\.log\}/)
  assert.match(binaryInstaller, /LOG_RETENTION_DAYS=\$\{MULTI_CPA_LOG_RETENTION_DAYS:-7\}/)
  assert.match(binaryInstaller, /rotate \$LOG_RETENTION_DAYS/)
  ...
  assert.match(sourceInstaller, /log_file=\$\{MULTI_CPA_LOG_FILE:-\/var\/log\/multi-cpa\.log\}/)
  assert.match(sourceInstaller, /log_retention_days=\$\{MULTI_CPA_LOG_RETENTION_DAYS:-7\}/)
  assert.match(sourceInstaller, /rotate \$log_retention_days/)
