...
export class Controller {
  ...
  async instanceRoute(request, response, url, rest) {
    ...
    if (['lock', 'unlock'].includes(action) && request.method === 'POST') { const instance = await this.instances.setLocked(instanceId, action === 'lock'); return writeJson(response, 200, publicInstance(instance, await this.instances.status(instanceId), request)) }
