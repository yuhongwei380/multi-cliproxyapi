...
CPA 版本下载不依赖 GitHub Release API：总控跟随 `/releases/latest` 重定向确定 tag，再直接下载对应的 Linux amd64 压缩包，并根据同一 release 的 `checksums.txt` 校验 SHA-256。`MULTI_CPA_GITHUB_TOKEN` 仅用于 CPA 管理页面资源的 GitHub API 请求，不是下载 CPA 版本的前置条件。
